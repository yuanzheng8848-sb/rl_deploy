...
def load_resnet10_params(agent, image_keys=("image",), public=True):
    ...
    if not public:  # if github repo is not public, load from local file
        ...
        # Monkey patch JAX ShapedArray to handle legacy "named_shape" argument
        # caused by loading newer JAX version with old pickle file
        import jax._src.core as jax_core
        original_init = jax_core.ShapedArray.__init__

        def patched_init(self, shape, dtype, weak_type=False, named_shape=None, properties=None, **kwargs):
             # Ignore named_shape and properties if passed from old pickle
             # Pass other kwargs (like sharding) to original_init
             original_init(self, shape, dtype, weak_type=weak_type, **kwargs)
