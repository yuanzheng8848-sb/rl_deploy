...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--dual",
        action="store_true",
        help="Auto-detect two 3D mice and print both event paths.",
    )
...
def auto_detect_dual_event_devices(verbose: bool = True):
    try:
        from evdev import InputDevice, list_devices
    except ImportError:
        print("[ERROR] Missing dependency: evdev")
    ...
    candidates.sort(key=lambda x: x[0], reverse=True)
    unique_paths = []
    for _score, path, name in candidates:
    ...
    for path in unique_paths:
        dev = inspect_event_device(path)
        if dev is not None:
            devices.append(dev)

    if verbose and devices:
        print("=== Auto-Detected Dual 3D Mouse Ports ===")
        if len(devices) >= 1:
            print(f"left_spacemouse_event_path={devices[0].path}")
        if len(devices) >= 2:
            print(f"right_spacemouse_event_path={devices[1].path}")
    return devices
...
def main() -> int:
    ...
    if args.dual:
        devices = auto_detect_dual_event_devices(verbose=True)
        return 0 if len(devices) >= 1 else 1
