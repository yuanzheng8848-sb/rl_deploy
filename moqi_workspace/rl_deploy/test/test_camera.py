...
- CAMERA_CONFIGS contains head USB camera by-id path (Global Shutter Camera)
...
# Global Shutter Camera, fixed by-id path so /dev/videoN reordering does not break it.
HEAD_USB_CAMERA_DEVICE = "/dev/v4l/by-id/usb-Global_Shutter_Camera_Global_Shutter_Camera_01.00.00-video-index0"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--auto-head-port",
        action="store_true",
        help="Auto-detect top USB camera from /dev/video* (recommended when by-id path is missing)",
    )
