...
def main():
    ...
    EXTRA_FAILURE_DIR = "/home/sj/Desktop/zy/moqi_workspace/rl_deploy/classifier/extra_images"
    CHECKPOINT_PATH = "/home/sj/Desktop/zy/moqi_workspace/rl_deploy/classifier/classifier_ckpt" # Relative path in current dir
    ...
    if os.path.exists(EXTRA_IMG_DIR):
        ...
        # 1. Extra Success
        path_pattern = os.path.join(EXTRA_IMG_DIR, "success", "**", "*.jpg")
        extra_success_paths = sorted(glob.glob(path_pattern, recursive=True))
        for path in tqdm(extra_success_paths, desc="Loading Extra Success"):
            try:
                img = cv2.imread(path)
                if img is None: continue
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                img = cv2.resize(img, (128, 128))
                extra_success_images.append(img)
            except Exception: pass

        # 2. Extra Failure
        path_pattern = os.path.join(EXTRA_IMG_DIR, "failure", "**", "*.jpg")
        extra_failure_paths = sorted(glob.glob(path_pattern, recursive=True))
        for path in tqdm(extra_failure_paths, desc="Loading Extra Failure"):
            try:
                img = cv2.imread(path)
                if img is None: continue
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    ...
    print(f"Loaded {len(extra_success_images)} extra SUCCESS images.")
    print(f"Loaded {len(extra_failure_images)} extra FAILURE images.")
    
    # Combine for checks
    total_images = len(success_images) + len(failure_images) + len(extra_success_images) + len(extra_failure_images)
    ...
    acc_orig_success = evaluate_set(success_images, "Original Success", 1)
    acc_orig_failure = evaluate_set(failure_images, "Original Failure", 0)
    acc_extra_success = evaluate_set(extra_success_images, "Extra Success", 1)
    acc_extra_failure = evaluate_set(extra_failure_images, "Extra Failure", 0)
    
    # Overall
    total_correct = (acc_orig_success * len(success_images) + 
                     acc_orig_failure * len(failure_images) + 
                     acc_extra_success * len(extra_success_images) + 
                     acc_extra_failure * len(extra_failure_images))
    if total_images > 0:
    ...
    print("FINAL RESULTS")
    print("="*40)
    print(f"Original Success Accuracy: {acc_orig_success:.4f}")
    print(f"Original Failure Accuracy: {acc_orig_failure:.4f}")
    print(f"Extra Success Accuracy:    {acc_extra_success:.4f}")
    print(f"Extra Failure Accuracy:    {acc_extra_failure:.4f}")
    print("-"*40)
