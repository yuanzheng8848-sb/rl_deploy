...
FLAGS = flags.FLAGS
flags.DEFINE_string("checkpoint_path", "../classifier_ckpt_cam1_v2", "Path to save checkpoint")
...
def main(_):
    ...
    # --- Load Extra Failure Images ---
    extra_failure_dir = "/home/peter/Desktop/zy/moqi_workspace/rl_deploy/recording_20260122_203118"
    if os.path.exists(extra_failure_dir):
        ...
        for path in extra_paths:
            try:
                ...
                if img is None:
                    continue
                # Assuming these are BGR from cv2.imwrite
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                img = cv2.resize(img, (128, 128))
                img = img[None, ...]
                ...
                print(f"Error loading extra image {path}: {e}")
        print(f"Added {count_extra} extra failure images.")
    else:
        print(f"Extra failure directory not found: {extra_failure_dir}")
