...
# --- 导入 IK 求解�?(可选但推荐) ---
# 用于�?Gym 的笛卡尔指令转换�?Controller 的关节指�?try:
...
class OpenArmServer:
    ...
    def move_ik(self, target_pose_flat, duration=1, gripper_pos=None):
        ...
        Args:
            target_pose_flat: list/array, length 14. [L_pos(3), L_quat(4), R_pos(3), R_quat(4)]
                              Env 发送格式通常�?[px, py, pz, qx, qy, qz, qw]
    ...
    def move_joints(self, joints, duration=3, gripper_pos=None):
        ...
        移动关节到指定位�?(平滑移动)
        Args:
            joints: 14维关节角度列�?[left_7, right_7]
            duration: 移动时间 (�?
            gripper_pos: 2维夹爪位置列�?[left, right] (Optional)
        """
