...
try:
    from openarm_ik import utils as utils_module
...
class Triangle:
    ...
    def get_current_ee_pose(self):
        # 为了�?VR 调用的， 从原本的 qw qx qy qz x y z => x y z qx qy qz qw
    ...
    def cal(self, B, theta, elbow_direction=1):
        # target_position，目标位�?np.array(3) x y z
        # theta 用于从多解中确认一�?
        start = time.time()
    ...
    def solve(self, 
        ...
        # target position 是在 shoulder坐标系下的，shoulder 就是A点位�?
        ...
        theta_step_size = 0.05  # �?.1减小�?.05，提高搜索精�?
        ...
        iteration_count = 0
        max_iterations = 80  # �?0增加�?0，配合更小的步长
